"""
Real, one-shot test: does `wan2.6-i2v-us` accept duration=15 (the enum's
documented upper value) on this account? `derisk/test_wan26_longer_duration.py`
(earlier this session) already confirmed duration=10 works for real
(10.02s measured clip, ffprobe-verified) -- this is the SAME script, with
ONLY the requested duration changed to 15, per the video-gen-fidelity
story-arc task brief: "verify it for real, once, before building the
pipeline around it."
...
production code path, identical in every other respect to the duration=10
script.

Usage (from backend/):
    python -m derisk.test_wan26_15s_duration
"""
...
JOB_ID = "wan26-duration-spike-15s"
...
async def main() -> int:
    ...
    try:
        ...
        (OUTPUTS_DIR / "wan26_15s_duration_result.json").write_text(json.dumps(result, indent=2))
        return 1

    import urllib.request
    local_path = str(OUTPUTS_DIR / "wan26_15s_test.mp4")
    ...
    OUTPUTS_DIR.mkdir(parents=True, exist_ok=True)
    out_path = OUTPUTS_DIR / "wan26_15s_duration_result.json"
