...
- [ ] `[BRAIN]` **Justification Validator** (deterministic: verbatim quote check, `truth_fact_id` exists, `treatment_ref` matches, stoplist reject; called synchronously from inside the Shot-List Agent between its two calls, per the interface handoff above — signature: `validate_justifications(...) -> list[ValidationResult]`)
...
- [ ] `[BRAIN]` **Budget Gate** (deterministic cap check, one loop-back to Shot-List Agent if over cap — priority-ordered: downgrade resolution/retry-reserve on lowest-weight shots first, then cut lowest-weight shot, uniform trim last resort — then enforce); grounding-weighted allocation formula per §5.7; write allocations to Budget Ledger table; emit `treatment_ready`/`budget_updated` events
